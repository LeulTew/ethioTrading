...
class TradingService {
  ...
  Future<void> executeTrade(Map<String, dynamic> tradeData) async {
    ...
    return _firestore.runTransaction((transaction) async {
      ...
      final totalCost =
          (tradeData['quantity'] as int) * (tradeData['price'] as num) +
      ...
      transaction.set(notificationDoc, {
        ...
        'type': 'trade',
        'title':
            '${tradeData['side'] == 'buy' ? 'Buy' : 'Sell'} Order Executed',
        'description':
            'Your ${tradeData['side']} order for ${tradeData['quantity']} shares of ${tradeData['symbol']} has been executed at ${tradeData['price']}',
        'timestamp': FieldValue.serverTimestamp(),
  ...
  void _updatePortfolioForBuy(
      List<Map<String, dynamic>> portfolio, Map<String, dynamic> tradeData) {
    final existingPosition = portfolio.firstWhere(
      (p) => p['symbol'] == tradeData['symbol'],
      orElse: () =>
          {'symbol': tradeData['symbol'], 'quantity': 0, 'avgPrice': 0.0},
    );
